...
class Site_View_Helper_Renderx extends Zend_View_Helper_Abstract implements Site_View_Helper_MarkupInterface
{
    ...
    public function renderx($options = array())
    {
        ...
        if (isset($options['template'])) {
            ...
            $templatePropResource = null;
            $templatePropClass = null;
            if (isset($options['templateProp'])) {
                $templatePropResource = $options['templateProp'];
                $templatePropClass = $options['templateProp'];
            }
            if (isset($options['templatePropResource'])) {
                $templatePropResource = $options['templatePropResource'];
            }
            if (isset($options['templatePropClass'])) {
                $templatePropClass = $options['templatePropClass'];
            }
            // try to query a template name
            $queriedTemplate = $this->selectTemplate($templatePropResource, $templatePropClass);
     ...
     */
    private function selectTemplate($resourceProp = null, $classProp = null)
    {
        if ($resourceProp === null) {
            $resourceProp = self::TEMPLATE_PROP_RESOURCE;
        }
        if ($classProp === null) {
            $classProp = self::TEMPLATE_PROP_CLASS;
        }
        $description  = $this->getDescription();
        $templateName = null;

        // if we have specific template on the resource, use it
        if (isset($description[$resourceProp][0]['value'])) {
            $templateName   = $description[$resourceProp][0]['value'];
        } else {
            $mappings = $this->getMappings($classProp);
        ...
        if ($templateName === null) {
            ...
            while (count($classes) > 0) {
                foreach($classes as $classUri) {
                    ...
                    if ($superClass !== null) {
                        if (isset($mappings[$superClass])) {
                            $templateName = $mappings[$superClass];
                            break;
                        }
     ...
     */
    public function getMappings($classProp = null)
    {
        if ($classProp === null) {
            $classProp = self::TEMPLATE_PROP_CLASS;
        }
        if (!isset($this->_mappings[$classProp])) {
            ...
            $this->_mappings[$classProp] = array();
            foreach ($result as $mapping) {
                $uri      = $mapping['class'];
                $template = $mapping['template'];
                $this->_mappings[$classProp][$uri] = $template;
            }
        }

        return $this->_mappings[$classProp];
    }
